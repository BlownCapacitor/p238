...
#defines api route and connection method
@api.route('/login', methods=['POST'])

#gets email and password for login, and checks that they match.
def login():
    try:
...
#logs the user out by removing their email and password from the sessioh
def logout():
    try:
        session["email"] = None
...
#Gets address data from the user
def add_address():
    try:
...
#gets data and uses it to create an order
def create_order():
    try:
        user_email = session.get("email")
        user_query = f"select * from users where email='{user_email}';"
...
#This is the code for the help ticket system which we exploited in
# one of the previous classes. It fetches the data from an input form, and allows the
...
#Saves the downloaded attachment (virus, in our case) to the computer
def download(filename):
    return send_file(os.path.join(UPLOAD_FOLDER, filename), as_attachment=True)

@api.route("/search-order")

#uses select and join queries to seach for orders
def search_order():
...
#the code for the SQL editor. It fetches the queries and executes them
def execute():
    try:
...
#Searches for customers based on ID
def get_customer():
    try:
