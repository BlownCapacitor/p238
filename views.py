...
#This blueprint is used to define URLs, like in line 13
...
@cross_origin()
#Fetches the products from the database and displays them in the dashboard page
def dashboard():
    try:
...
#Fetches user data and displays it on the profile page
def profile():
    try:
...
@views.route('/order')
@cross_origin()
#fetches orders and displays them in the order history page
def order():
    try:
